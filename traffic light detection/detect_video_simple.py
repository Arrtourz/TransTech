...
# Variables to record state changes
state_timeline = []
...
def process_frame(frame, detection_box, conf_threshold=0.3):
    ...
    # Add state information at the bottom of the video
...
def main(video_path):
    ...
    # Add the last state to the timeline
    if area_state != "Unknown":
        state_timeline.append((area_state, current_state_start, frame_count - current_state_start))
    ...
    # Save timeline to CSV file and output content
    video_name = os.path.splitext(os.path.basename(video_path))[0]
